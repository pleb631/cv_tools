...
def process_batch(detections, labels):
    ...
    correct_class = labels[:, 5:6] == detections[:, 5]
    ...
    for i in range(len(iouv)):
        ...
        if x[0].shape[0]:
            matches = (
                np.concatenate((np.stack(x, 1), iou[x[0], x[1]][:, None]), 1)
            )  # [label, detect, iou]
            if x[0].shape[0] > 1:
...
def ap_per_class(tp,
    ...
    p1,r1,f11= p.copy(),r.copy(),f1.copy()
    p, r, f1 = p[:, i], r[:, i], f1[:, i]
    tp = (r * nt).round()  # true positives
    fp = (tp / (p + eps) - tp).round()  # false positives
    return tp, fp, p, r, f1, ap, unique_classes.astype(int),p1,r1,f11,px, py,i
...
class DetMetrics():
    ...
    def __init__(self, save_dir=Path('.'), plot=False, on_plot=None, names=(),**kwargs) -> None:
    ...
    def process(self, tp, conf, pred_cls, target_cls):
        ...
        *results,p,r,f1,px,py,index = ap_per_class(tp,
                               conf,
                               pred_cls,
                               target_cls,
                               plot=self.plot,
