...
class KPDModel(object):
    ...
    def detect(self, input):
        ...
        kpts = self.post_process(kpts, new_shape)

        return kpts
    
    def post_process(self,kpts,new_shape):
        if self.args.heat_maps:
            _,c,h,w  = kpts.shape
            kpts = cv2.resize(kpts.reshape(c,h,w).transpose(1,2,0), self.img_size)
            kpts = keypoints_from_heatmaps(kpts.transpose(2,0,1).reshape(1,c,*self.img_size),post_process='unbiased',use_udp=False)
            # heatmaps = kpts.reshape(c,h*w)
            # max_index= np.argmax(heatmaps,1)
            
            # index_h = max_index//w
            # index_w = max_index - index_h*w
            #kpts = np.concatenate((index_w.reshape(-1,1),index_h.reshape(-1,1)),1)
            
            kpts = kpts/self.img_size
        ...
        kpts = kpts*new_shape/[self.img_w,self.img_h]
        kpts = np.clip(kpts, 0, 1)    
        return kpts
