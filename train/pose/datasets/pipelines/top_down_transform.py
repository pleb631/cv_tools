...
from core.post_transforms import (affine_transform, fliplr_joints,
...
def plt(results):
            import cv2
            im = cv2.imread(results["image_file"])
            joints_2d = results['joints_3d'][:,:2]
            im = cv2.resize(im,(128,128))
            im1=im.copy()
            for p in joints_2d:
                cv2.circle(im1, (int(p[0]), int(p[1])), 3, (0, 0, 255), -1)
            cv2.imwrite("test1.jpg",im1)
            im1=im.copy()
            mask = results["target"]
            mask=np.max(mask,0)
            mask=cv2.resize(mask,im.shape[:2][::-1])
            im1[mask>0.8,0]=255
            cv2.imwrite("test2.jpg",im1)
            return 0         
