...
class ohemWingLoss(nn.Module):
    ...
    def criterion(self, pred, target,w=10.0, epsilon=2.0):

        y_pred,y_true = pred[target>1e-7], target[target>1e-7]
        x = y_true - y_pred
        c = w * (1.0 - math.log(1.0 + w / epsilon))
        absolute_x = torch.abs(x)
        loss = torch.where(w > absolute_x,
                             w * torch.log(1.0 + absolute_x / epsilon),
                             absolute_x - c)
        #loss = torch.mean(torch.sum(losses, axis=[1, 2]), axis=0)
        th_num=512
        th=0.15
        loss = loss[loss.argsort(descending=True)]
        num = max(torch.sum(loss>th),th_num)
        loss = loss[:num]
...
class WingLoss(nn.Module):
    ...
    def criterion(self, pred, target):
        ...
        # th_num=512
        # th=0.15
        # loss = losses[losses.argsort(descending=True)]
        # num = max(torch.sum(loss>th),th_num)
        # loss = loss[:num]
        # loss = torch.mean(loss)
