...
from core.top_down_eval import (keypoint_auc, keypoint_epe,
                                                  keypoint_nme,
...
class Kpt2dSviewRgbImgTopDownDataset(Dataset, metaclass=ABCMeta):
    ...
    def _report_metric(self,
        ...
        with open(res_file, 'r') as fin:
            preds = json.load(fin)
        assert len(preds) == len(self.db),f"{len(self.db)},{len(preds)}"
    ...
    def plot(self,result):
        import cv2
        img = cv2.imread(result['img_metas'].data['image_file'])
        kpts = result['target']
        img = cv2.resize(img,(112,112))
        color = [(255,0,0),(0,0,0),(0,255,0),(0,0,255),(128,255,0)]
        for i,p in enumerate(kpts):
            x,y=p
            img = cv2.circle(img, (int(x*112), int(y*112)), 1, (255,0,0), -1)
        #img=cv2.cvtColor(img,cv2.COLOR_RGB2BGR)
        img = cv2.resize(img,(224,244))
        cv2.imwrite('test_ori.jpg',img)
